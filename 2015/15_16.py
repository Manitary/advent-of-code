...
def findAunt(target, part=1):
    for i, aunt in aunts.items():
        if all(compareItem(thing, amount, target[thing], part) for thing, amount in aunt.items()):
            return i
...
pattern = r"(\w+): (\d+)"
compile(pattern)
aunts = {i + 1 : {p[0]: int(p[1]) for p in findall(pattern, row)} for i, row in enumerate(data)}
