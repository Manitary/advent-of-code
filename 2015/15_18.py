...
def neighbours(panel, x, y):
    return sum((panel[b][a] for a, b in product(*(range(n - 1, n + 2) for n in (x, y))))) - panel[y][x]
...
def switchLights(panel, part=1):
    return [[newState(panel, x, y, c, part) for x, c in enumerate(row)] for y, row in enumerate(panel)]
