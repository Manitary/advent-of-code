...
def prime_powers(n):
    ...
    for c in accumulate(chain([2, 1, 2], cycle([2,4]))):
        ...
        d, p = (), c
        while not n % c:
            n, p, d = n//c, p*c, d + (p,) # Find all the powers of c dividing n, and store them in d. Divide n by the maximum power of c that divides it.
        yield d
    if n > 1:
...
def factors(n):
    ...
    for e in prime_powers(n):
        r += [a*b for a in r for b in e] # For each new prime power divisor p^k of n, add to the list all the possible combinations of products q_1^a_1 * ... * q_m^a_m for other primes q_i < p
    return r
