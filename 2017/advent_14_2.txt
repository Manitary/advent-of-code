...
procedure TurnOffAdjacent(~grid,line,column)
	print line, column;
	grid[line][column]:=false;
	for x in [Maximum(line-1,1)..Minimum(line+1,128)] do
		for y in [Maximum(column-1,1)..Minimum(column+1,128)] do
			if Abs(line-x)+Abs(column-y) eq 1 and grid[x][y] then
				TurnOffAdjacent(~grid,x,y);
			end if;
		end for;
	end for;
end procedure;
...
for line in [1..128] do
	...
	for i in [1..32] do
		for j in [1..4] do
			Append(~grid[line],squares[line][i][j]);
		end for;
	end for;
end for;
...
regions:=0;
for line in [1..128] do
	for column in [1..128] do
		if grid[line][column] then
			regions+:=1;
			TurnOffAdjacent(~grid,line,column);
		end if;
	end for;
end for;
regions;